    ...
    <div className="relative flex flex-col items-center bg-transparent p-4 pt-6 mb-4">
      ...
      <div className="absolute top-3 right-3">
        <button
          ...
          aria-label="환경설정"
        >
          <Cog6ToothIcon className="w-5 h-5 text-gray-500 dark:text-gray-200" />
        </button>
        {/* 환경설정 드롭다운 */}
        {showSettings && (
      ...
      {/* 닉네임 */}
      <div className="text-base font-bold text-gray-800 dark:text-gray-100 mb-1">{name}</div>
      <div className="text-xs text-gray-500 dark:text-gray-300">{session.user.email}</div>
    </div>
  )
} 