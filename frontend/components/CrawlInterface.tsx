    ...
    <div className="space-y-6 p-6">
      ...
      {crawlSites && (
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
            ...
            {crawlSites.sites.map((site: any, index: number) => (
              <div key={index} className="flex items-center">
                <span className={`w-2 h-2 rounded-full mr-2 ${
                  site.enabled ? 'bg-blue-500' : 'bg-gray-300'
                }`}></span>
          ...
          <button
            ...
            className="w-full mt-3 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors duration-200 text-sm font-medium"
          >
      ...
      {autoError && (
        <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
          <p className="text-gray-800">{autoError}</p>
        </div>
      )}

      {autoTaskId && (
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-blue-800 font-medium">자동 크롤링 작업이 시작되었습니다!</p>
          <p className="text-blue-600 text-sm mt-1">Task ID: {autoTaskId}</p>
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="space-x-2 flex">
              <button
                ...
                className="h-8 px-2 flex items-center bg-blue-500 text-white text-xs rounded-lg hover:bg-blue-600 disabled:opacity-50 transition-colors duration-200"
              >
                {isRefreshing ? (
              ...
              <button
                ...
                className="h-8 px-2 flex items-center bg-gray-700 text-white text-xs rounded-lg hover:bg-gray-800 transition-colors duration-200"
              >
          ...
          {(isLoading || isAutoLoading) && (
            <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded">
              <p className="text-blue-800 font-medium"> 진행중인 작업:</p>
              {isLoading && (
                <p className="text-blue-700 text-sm"> 수동 크롤링: {rootUrl}</p>
              )}
              {isAutoLoading && (
                <p className="text-blue-700 text-sm"> 자동 크롤링: JSON 파일 사이트들</p>
              )}
                  ...
                  {dbStatus.recent_updates.map((item: any, index: number) => {
                    ...
                    return (
                      <div key={index} className={`p-2 rounded text-xs border ${
                        isJsonSite ? 'bg-gray-50 border-gray-200' : 'bg-blue-50 border-blue-200'
                      }`}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`px-2 py-1 rounded text-xs ${
                            isJsonSite ? 'bg-gray-700 text-white' : 'bg-blue-600 text-white'
                          }`}>
                            {isJsonSite ? 'JSON' : '수동'}
                          </span>
      ...
      {taskId && (
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-blue-800 font-medium">크롤링 작업이 시작되었습니다!</p>
          <p className="text-blue-600 text-sm mt-1">Task ID: {taskId}</p>
          <p className="text-blue-600 text-sm mt-2">
            크롤링이 완료되면 챗봇에서 질문할 수 있습니다.
          </p>
        </div>
      ...
      <div className="p-4 bg-white border border-blue-300 rounded-lg">
        <h3 className="font-medium text-black mb-2">크롤링 안내</h3>
